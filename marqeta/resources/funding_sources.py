...
from marqeta.response_models.funding_account_response_model import (
...
from marqeta.response_models.program_funding_source_response import (
    ProgramFundingSourceResponse,
)
from marqeta.response_models.gateway_program_funding_source_response import (
...
class FundingSourcesCollection(object):
    ...
    def __init__(self, client):
        ...
        self.payment_card = PaymentCard(
            Collection(self.client, PaymentCardResponseModel)
        )
        self.program_gateway = ProgramGateway(
            Collection(self.client, GatewayProgramFundingSourceResponse)
        )
    ...
    def page_for_user(self, user_token, count=5, start_index=0, params=None):
        ...
        return self.collections.page(
            endpoint=self._endpoint + "/user/{}".format(user_token),
            count=count,
    ...
    def page_for_business(self, business_token, count=5, start_index=0, params=None):
        ...
        """
        return self.collections.page(
            endpoint=self._endpoint + "/business/{}".format(business_token),
            count=count,
            start_index=start_index,
            query_params=params,
        )
    ...
    def stream_for_user(self, user_token, params=None):
        ...
        return self.collections.stream(
            endpoint=self._endpoint + "/user/{}".format(user_token), query_params=params
        )
...
class FundingSourceMakeDefualt(FundingSourcesCollection):
    ...
    def make_default(self):
        return self.collections.save(
            data={}, endpoint="fundingsources/{}/default".format(self.token)
        )
...
class AchContext(object):
    ...
    def verification_amounts(self, params=None):
        return self.collections.find(
            endpoint="fundingsources/ach/{}/verificationamounts".format(self.token),
            query_params=params,
        )
...
class Ach(object):
    ...
    def find(self, funding_token, params=None):
        return self.collections.find(
            endpoint=self._endpoint + "/{}".format(funding_token), query_params=params
        )
