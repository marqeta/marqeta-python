...
from marqeta.response_models.digital_wallet_token_transition_response import (
...
class DigitalWalletTokensCollection(object):
    ...
    def page_for_card(self, user_token, count=5, start_index=0, params=None):
        ...
        """
        return self.collections.page(
            endpoint=self._endpoint + "/card/{}".format(user_token),
            count=count,
            start_index=start_index,
            query_params=params,
        )
    ...
    def stream_for_card(self, user_token, params=None):
        ...
        """
        return self.collections.stream(
            endpoint=self._endpoint + "/card/{}".format(user_token), query_params=params
        )
    ...
    def find_show_pan(self, card_token, params=None):
        ...
        """
        return self.collections.find(
            endpoint=self._endpoint + "/{}/showtokenpan".format(card_token),
            query_params=params,
        )
...
class DigitalWalletTokensContext(DigitalWalletTokensCollection):
    ...
    def __init__(self, token, client):
        ...
        self.transitions = self.Transitions(
            self.token, Collection(client, DigitalWalletTokenTransitionResponse)
        )
