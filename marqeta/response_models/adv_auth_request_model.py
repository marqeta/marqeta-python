...
from marqeta.response_models.advanced_auth_card_acceptor_model import (
...
class AdvAuthRequestModel(object):
    ...
    def digital_wallet_token_device_info(self):
        if "digital_wallet_token_device_info" in self.json_response:
            return DigitalWalletTokenDevice(
                self.json_response["digital_wallet_token_device_info"]
            )
    ...
    def digital_wallet_token_wallet_provider_info(self):
        if "digital_wallet_token_wallet_provider_info" in self.json_response:
            return DigitalWalletTokenWalletProvider(
                self.json_response["digital_wallet_token_wallet_provider_info"]
            )
    ...
    def __repr__(self):
        return (
            "<Marqeta.response_models.adv_auth_request_model.AdvAuthRequestModel>"
            + self.__str__()
        )
