...
from marqeta.response_models.card_security_code_verification import (
...
from marqeta.response_models.cardholder_authentication_data import (
...
class TransactionModel(object):
    ...
    def incremental_authorization_transaction_tokens(self):
        return self.json_response.get(
            "incremental_authorization_transaction_tokens", None
        )
    ...
    def offer_orders(self):
        if "offer_orders" in self.json_response:
            return [
                OfferOrderResponse(val) for val in self.json_response["offer_orders"]
            ]
    ...
    def card_security_code_verification(self):
        if "card_security_code_verification" in self.json_response:
            return CardSecurityCodeVerification(
                self.json_response["card_security_code_verification"]
            )
    ...
    def cardholder_authentication_data(self):
        if "cardholder_authentication_data" in self.json_response:
            return CardholderAuthenticationData(
                self.json_response["cardholder_authentication_data"]
            )

    def __repr__(self):
        return (
            "<Marqeta.response_models.transaction_model.TransactionModel>"
            + self.__str__()
        )
