...
from marqeta.response_models.wallet_provider_card_on_file import (
...
class ProvisioningControls(object):
    ...
    def wallet_provider_card_on_file(self):
        if "wallet_provider_card_on_file" in self.json_response:
            return WalletProviderCardOnFile(
                self.json_response["wallet_provider_card_on_file"]
            )
    ...
    def __repr__(self):
        return (
            "<Marqeta.response_models.provisioning_controls.ProvisioningControls>"
            + self.__str__()
        )
