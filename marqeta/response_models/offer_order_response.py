...
from marqeta.response_models.offer_order_aggregated_balances import (
...
class OfferOrderResponse(object):
    ...
    def order_aggregated_balances(self):
        if "order_aggregated_balances" in self.json_response:
            return OfferOrderAggregatedBalances(
                self.json_response["order_aggregated_balances"]
            )
    ...
    def __repr__(self):
        return (
            "<Marqeta.response_models.offer_order_response.OfferOrderResponse>"
            + self.__str__()
        )
