...
from marqeta.response_models.identification_request_model import (
...
from marqeta.response_models.business_proprietor_request_model import (
...
class BusinessCardHolderUpdateModel(object):
    ...
    def identifications(self):
        if "identifications" in self.json_response:
            return [
                IdentificationRequestModel(val)
    ...
    def incorporation(self):
        if "incorporation" in self.json_response:
            return BusinessIncorporationRequestModel(
    ...
    def proprietor_or_officer(self):
        if "proprietor_or_officer" in self.json_response:
            return BusinessProprietorRequestModel(
                self.json_response["proprietor_or_officer"]
            )

    def __repr__(self):
        return (
            "<Marqeta.response_models.business_card_holder_update_model.BusinessCardHolderUpdateModel>"
            + self.__str__()
        )
